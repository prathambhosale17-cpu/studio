
const en = {
  'Manage IDs': 'Manage IDs',
  'Verify ID': 'Verify ID',
  'Download Card': 'Download Card',
  'Create New ID Card': 'Create New ID Card',
  'Fill in the details to generate a new digital ID card.': 'Fill in the details to generate a new digital ID card.',
  'Full Name': 'Full Name',
  'Date of Birth': 'Date of Birth',
  'Gender': 'Gender',
  'Male': 'Male',
  'Female': 'Female',
  'Other': 'Other',
  'Address': 'Address',
  'Photo': 'Photo',
  'Click to upload': 'Click to upload',
  'Generate ID Card': 'Generate ID Card',
  'My ID Cards': 'My ID Cards',
  'A list of all the digital ID cards you have created.': 'A list of all the digital ID cards you have created.',
  "You haven't created any ID cards yet.": "You haven't created any ID cards yet.",
  'Search by ID Number...': 'Search by ID Number...',
  'No cards found for your search.': 'No cards found for your search.',
  'siteTitle': 'Yojana Mitra • Maharashtra',
  'siteTag': 'Schemes & Community Help',
  'heroTitle': 'Find the right scheme in minutes',
  'heroSub': 'Answer a few questions to see schemes you can apply for.',
  'heroNote': 'No login needed. Anonymous posting allowed.',
  'finderTitle': 'Smart Scheme Finder',
  'labelDistrict': 'District',
  'labelCrop': 'Crop',
  'labelLand': 'Land Size',
  'labelIrr': 'Irrigation',
  'labelAge': 'Age',
  'labelKyc': 'KYC & Bank Link',
  'btnShowEligible': 'Show Eligible Schemes',
  'btnClear': 'Clear',
  'btnFindSchemes': 'Find Schemes',
  'btnAsk': 'Ask a Doubt',
  'qaTitle': "Farmers’ Q&A",
  'labelQDistrict': 'District',
  'labelQCat': 'Category',
  'labelQTitle': 'Title',
  'labelQBody': 'Describe your problem',
  'lblUnanswered': 'Unanswered',
  'print': 'Print',
  'apply': 'Apply Online',
  'footerTxt': 'Best-effort information. Verify with official sources.',
  'details': 'Details',
  'noSchemes': 'No matching schemes found. Try changing filters.',
  'noPosts': 'No posts yet.',
  'captcha': 'Captcha:',
  'copy': 'Copy',
  'copied': 'Copied!',
  'downloadPdf': 'Download PDF',
  'close': 'Close',
  'whatYouGet': 'What you get',
  'eligibility': 'Eligibility',
  'howToApply': 'How to apply',
  'docsYouNeed': 'Documents you need',
  'postDoubt': 'Post Doubt',
  'wrongCaptcha': 'Wrong captcha',
  'postReported': 'Reported. Post hidden.',
  'markSolved': 'Mark Solved',
  'report': 'Report',
};

const hi = {
  'Manage IDs': 'आईडी प्रबंधित करें',
  'Verify ID': 'आईडी सत्यापित करें',
  'Download Card': 'कार्ड डाउनलोड करें',
  'Create New ID Card': 'नया आईडी कार्ड बनाएं',
  'Fill in the details to generate a new digital ID card.': 'नया डिजिटल आईडी कार्ड बनाने के लिए विवरण भरें।',
  'Full Name': 'पूरा नाम',
  'Date of Birth': 'जन्म की तारीख',
  'Gender': 'लिंग',
  'Male': 'पुरुष',
  'Female': 'महिला',
  'Other': 'अन्य',
  'Address': 'पता',
  'Photo': 'तस्वीर',
  'Click to upload': 'अपलोड करने के लिए क्लिक करें',
  'Generate ID Card': 'आईडी कार्ड बनाएं',
  'My ID Cards': 'मेरे आईडी कार्ड',
  'A list of all the digital ID cards you have created.': 'आपके द्वारा बनाए गए सभी डिजिटल आईडी कार्डों की एक सूची।',
  "You haven't created any ID cards yet.": "आपने अभी तक कोई आईडी कार्ड नहीं बनाया है।",
  'Search by ID Number...': 'आईडी नंबर से खोजें...',
  'No cards found for your search.': 'आपकी खोज के लिए कोई कार्ड नहीं मिला।',
  'siteTitle': 'योजना मित्र • महाराष्ट्र',
  'siteTag': 'योजनाएं और सामुदायिक सहायता',
  'heroTitle': 'मिनटों में सही योजना खोजें',
  'heroSub': 'आप जिन योजनाओं के लिए आवेदन कर सकते हैं, उन्हें देखने के लिए कुछ सवालों के जवाब दें।',
  'heroNote': 'लॉगिन की आवश्यकता नहीं है। अनाम पोस्टिंग की अनुमति है।',
  'finderTitle': 'स्मार्ट योजना खोजक',
  'labelDistrict': 'जिला',
  'labelCrop': 'फ़सल',
  'labelLand': 'भूमि का आकार',
  'labelIrr': 'सिंचाई',
  'labelAge': 'आयु',
  'labelKyc': 'केवाईसी और बैंक लिंक',
  'btnShowEligible': 'योग्य योजनाएं दिखाएं',
  'btnClear': 'साफ़ करें',
  'btnFindSchemes': 'योजनाएं खोजें',
  'btnAsk': 'संदेह पूछें',
  'qaTitle': "किसानों के प्रश्नोत्तर",
  'labelQDistrict': 'जिला',
  'labelQCat': 'श्रेणी',
  'labelQTitle': 'शीर्षक',
  'labelQBody': 'अपनी समस्या का वर्णन करें',
  'lblUnanswered': 'अनुत्तरित',
  'print': 'प्रिंट',
  'apply': 'ऑनलाइन आवेदन करें',
  'footerTxt': 'सर्वोत्तम-प्रयास जानकारी। आधिकारिक स्रोतों से सत्यापित करें।',
  'details': 'विवरण',
  'noSchemes': 'कोई मेल खाने वाली योजना नहीं मिली। फ़िल्टर बदलने का प्रयास करें।',
  'noPosts': 'अभी तक कोई पोस्ट नहीं है।',
  'captcha': 'कैप्चा:',
  'copy': 'कॉपी करें',
  'copied': 'कॉपी किया गया!',
  'downloadPdf': 'पीडीएफ डाउनलोड करें',
  'close': 'बंद करें',
  'whatYouGet': 'आपको क्या मिलता है',
  'eligibility': 'पात्रता',
  'howToApply': 'आवेदन कैसे करें',
  'docsYouNeed': 'आवश्यक दस्तावेज़',
  'postDoubt': 'संदेह पोस्ट करें',
  'wrongCaptcha': 'गलत कैप्चा',
  'postReported': 'रिपोर्ट किया गया। पोस्ट छिपी हुई है।',
  'markSolved': 'हल के रूप में चिह्नित करें',
  'report': 'रिपोर्ट करें',
};

const bn = {
    'Manage IDs': 'আইডি পরিচালনা করুন',
    'Verify ID': 'আইডি যাচাই করুন',
    'Download Card': 'কার্ড ডাউনলোড করুন',
    'Create New ID Card': 'নতুন আইডি কার্ড তৈরি করুন',
    'Fill in the details to generate a new digital ID card.': 'একটি নতুন ডিজিটাল আইডি কার্ড তৈরি করতে বিবরণ পূরণ করুন।',
    'Full Name': 'পুরো নাম',
    'Date of Birth': 'জন্ম তারিখ',
    'Gender': 'লিঙ্গ',
    'Male': 'পুরুষ',
    'Female': 'মহিলা',
    'Other': 'অন্যান্য',
    'Address': 'ঠিকানা',
    'Photo': 'ছবি',
    'Click to upload': 'আপলোড করতে ক্লিক করুন',
    'Generate ID Card': 'আইডি কার্ড তৈরি করুন',
    'My ID Cards': 'আমার আইডি কার্ড',
    'A list of all the digital ID cards you have created.': 'আপনার তৈরি করা সমস্ত ডিজিটাল আইডি কার্ডের একটি তালিকা।',
    "You haven't created any ID cards yet.": "আপনি এখনও কোনো আইডি কার্ড তৈরি করেননি।",
    'Search by ID Number...': 'আইডি নম্বর দ্বারা অনুসন্ধান করুন...',
    'No cards found for your search.': 'আপনার অনুসন্ধানের জন্য কোন কার্ড পাওয়া যায়নি।',
    'siteTitle': 'যোজনা মিত্র • মহারাষ্ট্র',
    'siteTag': 'স্কিম এবং সম্প্রদায় সহায়তা',
    'heroTitle': 'মিনিটের মধ্যে সঠিক স্কিম খুঁজুন',
    'heroSub': 'আপনি আবেদন করতে পারেন এমন স্কিম দেখতে কয়েকটি প্রশ্নের উত্তর দিন।',
    'heroNote': 'কোন লগইন প্রয়োজন নেই। বেনামী পোস্টিং অনুমোদিত।',
    'finderTitle': 'স্মার্ট স্কিম ফাইন্ডার',
    'labelDistrict': 'জেলা',
    'labelCrop': 'ফসল',
    'labelLand': 'জমির আকার',
    'labelIrr': 'সেচ',
    'labelAge': 'বয়স',
    'labelKyc': 'কেওয়াইসি এবং ব্যাংক লিঙ্ক',
    'btnShowEligible': 'যোগ্য স্কিমগুলি দেখান',
    'btnClear': 'পরিষ্কার করুন',
    'btnFindSchemes': 'স্কিম খুঁজুন',
    'btnAsk': 'সন্দেহ জিজ্ঞাসা করুন',
    'qaTitle': "কৃষকদের প্রশ্নোত্তর",
    'labelQDistrict': 'জেলা',
    'labelQCat': 'বিভাগ',
    'labelQTitle': 'শিরোনাম',
    'labelQBody': 'আপনার সমস্যা বর্ণনা করুন',
    'lblUnanswered': 'উত্তরহীন',
    'print': 'প্রিন্ট',
    'apply': 'অনলাইনে আবেদন করুন',
    'footerTxt': 'সর্বোত্তম-প্রচেষ্টার তথ্য। অফিসিয়াল উত্স থেকে যাচাই করুন।',
    'details': 'বিস্তারিত',
    'noSchemes': 'কোনো মানানসই স্কিম পাওয়া যায়নি। ফিল্টার পরিবর্তন করার চেষ্টা করুন।',
    'noPosts': 'এখনও কোনো পোস্ট নেই।',
    'captcha': 'ক্যাপচা:',
    'copy': 'কপি',
    'copied': 'কপি করা হয়েছে!',
    'downloadPdf': 'পিডিএফ ডাউনলোড করুন',
    'close': 'বন্ধ করুন',
    'whatYouGet': 'আপনি কি পাবেন',
    'eligibility': 'যোগ্যতা',
    'howToApply': 'কিভাবে আবেদন করবেন',
    'docsYouNeed': 'প্রয়োজনীয় নথি',
    'postDoubt': 'সন্দেহ পোস্ট করুন',
    'wrongCaptcha': 'ভুল ক্যাপচা',
    'postReported': 'রিপোর্ট করা হয়েছে। পোস্ট লুকানো হয়েছে।',
    'markSolved': 'সমাধান হিসাবে চিহ্নিত করুন',
    'report': 'রিপোর্ট',
};

const te = {
    'Manage IDs': 'IDలను నిర్వహించండి',
    'Verify ID': 'IDని ధృవీకరించండి',
    'Download Card': 'కార్డును డౌన్‌లోడ్ చేయండి',
    'Create New ID Card': 'కొత్త ID కార్డును సృష్టించండి',
    'Fill in the details to generate a new digital ID card.': 'కొత్త డిజిటల్ ID కార్డును రూపొందించడానికి వివరాలను పూరించండి।',
    'Full Name': 'పూర్తి పేరు',
    'Date of Birth': 'పుట్టిన తేది',
    'Gender': 'లింగం',
    'Male': 'పురుషుడు',
    'Female': 'స్త్రీ',
    'Other': 'ఇతర',
    'Address': 'చిరునామా',
    'Photo': 'ఫోటో',
    'Click to upload': 'అప్‌లోడ్ చేయడానికి క్లిక్ చేయండి',
    'Generate ID Card': 'ID కార్డును రూపొందించండి',
    'My ID Cards': 'నా ID కార్డులు',
    'A list of all the digital ID cards you have created.': 'మీరు సృష్టించిన అన్ని డిజిటల్ ID కార్డుల జాబితా।',
    "You haven't created any ID cards yet.": "మీరు ఇంకా ఏ ID కార్డులను సృష్టించలేదు।",
    'Search by ID Number...': 'ఐడి నంబర్ ద్వారా శోధించండి...',
    'No cards found for your search.': 'మీ శోధనకు కార్డులు ఏవీ కనుగొనబడలేదు।',
    'siteTitle': 'యోజన మిత్ర • మహారాష్ట్ర',
    'siteTag': 'పథకాలు మరియు సమాజ సహాయం',
    'heroTitle': 'నిమిషాల్లో సరైన పథకాన్ని కనుగొనండి',
    'heroSub': 'మీరు దరఖాస్తు చేసుకోగల పథకాలను చూడటానికి కొన్ని ప్రశ్నలకు సమాధానం ఇవ్వండి.',
    'heroNote': 'లాగిన్ అవసరం లేదు. అనామక పోస్టింగ్ అనుమతించబడింది.',
    'finderTitle': 'స్మార్ట్ స్కీమ్ ఫైండర్',
    'labelDistrict': 'జిల్లా',
    'labelCrop': 'పంట',
    'labelLand': 'భూమి పరిమాణం',
    'labelIrr': 'నీటిపారుదల',
    'labelAge': 'వయస్సు',
    'labelKyc': 'కెవైసి మరియు బ్యాంక్ లింక్',
    'btnShowEligible': 'అర్హతగల పథకాలను చూపించు',
    'btnClear': 'క్లియర్',
    'btnFindSchemes': 'పథకాలను కనుగొనండి',
    'btnAsk': 'సందేహం అడగండి',
    'qaTitle': "రైతుల ప్రశ్నోత్తరాలు",
    'labelQDistrict': 'జిల్లా',
    'labelQCat': 'వర్గం',
    'labelQTitle': 'శీర్షిక',
    'labelQBody': 'మీ సమస్యను వివరించండి',
    'lblUnanswered': 'సమాధానం లేనివి',
    'print': 'ప్రింట్',
    'apply': 'ఆన్‌లైన్‌లో దరఖాస్తు చేసుకోండి',
    'footerTxt': 'ఉత్తమ-ప్రయత్న సమాచారం. అధికారిక మూలాల నుండి ధృవీకరించండి.',
    'details': 'వివరాలు',
    'noSchemes': 'సరిపోలే పథకాలు కనుగొనబడలేదు. ఫిల్టర్‌లను మార్చడానికి ప్రయత్నించండి.',
    'noPosts': 'ఇంకా పోస్ట్‌లు లేవు.',
    'captcha': 'క్యాప్చా:',
    'copy': 'కాపీ',
    'copied': 'కాపీ చేయబడింది!',
    'downloadPdf': 'PDF డౌన్‌లోడ్ చేయండి',
    'close': 'మూసివేయి',
    'whatYouGet': 'మీకు ఏమి లభిస్తుంది',
    'eligibility': 'అర్హత',
    'howToApply': 'ఎలా దరఖాస్తు చేయాలి',
    'docsYouNeed': 'అవసరమైన పత్రాలు',
    'postDoubt': 'సందేహాన్ని పోస్ట్ చేయండి',
    'wrongCaptcha': 'తప్పు క్యాప్చా',
    'postReported': 'నివేదించబడింది. పోస్ట్ దాచబడింది.',
    'markSolved': 'పరిష్కరించబడినట్లుగా గుర్తించండి',
    'report': 'నివేదిక',
};

const mr = {
    'Manage IDs': 'आयडी व्यवस्थापित करा',
    'Verify ID': 'आयडी सत्यापित करा',
    'Download Card': 'कार्ड डाउनलोड करा',
    'Create New ID Card': 'नवीन आयडी कार्ड तयार करा',
    'Fill in the details to generate a new digital ID card.': 'नवीन डिजिटल आयडी कार्ड तयार करण्यासाठी तपशील भरा।',
    'Full Name': 'पूर्ण नाव',
    'Date of Birth': 'जन्म तारीख',
    'Gender': 'लिंग',
    'Male': 'पुरुष',
    'Female': 'महिला',
    'Other': 'इतर',
    'Address': 'पत्ता',
    'Photo': 'फोटो',
    'Click to upload': 'अपलोड करण्यासाठी क्लिक करा',
    'Generate ID Card': 'आयडी कार्ड तयार करा',
    'My ID Cards': 'माझे आयडी कार्ड',
    'A list of all the digital ID cards you have created.': 'तुम्ही तयार केलेल्या सर्व डिजिटल आयडी कार्डांची सूची।',
    "You haven't created any ID cards yet.": "तुम्ही अद्याप कोणतेही आयडी कार्ड तयार केलेले नाही।",
    'Search by ID Number...': 'आयडी क्रमांकाद्वारे शोधा...',
    'No cards found for your search.': 'तुमच्या शोधासाठी कोणतेही कार्ड आढळले नाही।',
    'siteTitle': 'योजना मित्र • महाराष्ट्र',
    'siteTag': 'योजना व समुदाय मदत',
    'heroTitle': 'योग्य योजना काही मिनिटांत शोधा',
    'heroSub': 'थोडे प्रश्न उत्तर द्या आणि आपल्या पात्र योजना पहा.',
    'heroNote': 'लॉगिनची गरज नाही. गुप्तपणे प्रश्न विचारू शकता.',
    'finderTitle': 'स्मार्ट योजना शोधक',
    'labelDistrict': 'जिल्हा',
    'labelCrop': 'पीक',
    'labelLand': 'शेती क्षेत्र',
    'labelIrr': 'सिंचन',
    'labelAge': 'वय',
    'labelKyc': 'केवायसी व बँक लिंक',
    'btnShowEligible': 'पात्र योजना दाखवा',
    'btnClear': 'रीसेट',
    'btnFindSchemes': 'योजना शोधा',
    'btnAsk': 'शंका विचारा',
    'qaTitle': 'शेतकरी प्रश्नोत्तर',
    'labelQDistrict': 'जिल्हा',
    'labelQCat': 'विभाग',
    'labelQTitle': 'शीर्षक',
    'labelQBody': 'समस्या लिहा',
    'lblUnanswered': 'उत्तर नसलेले',
    'print': 'प्रिंट',
    'apply': 'ऑनलाइन अर्ज',
    'footerTxt': 'माहिती सर्वोत्तम प्रयत्नांवर आधारित. कृपया अधिकृत स्रोत तपासा.',
    'details': 'तपशील',
    'noSchemes': 'जुळणाऱ्या योजना आढळल्या नाहीत. फिल्टर बदलून प्रयत्न करा.',
    'noPosts': 'अद्याप पोस्ट नाहीत.',
    'captcha': 'कॅप्चा:',
    'copy': 'कॉपी',
    'copied': 'कागदपत्रांची यादी कॉपी केली',
    'downloadPdf': 'PDF डाउनलोड करा',
    'close': 'बंद करा',
    'whatYouGet': 'तुम्हाला काय मिळेल',
    'eligibility': 'पात्रता',
    'howToApply': 'अर्ज कसा करावा',
    'docsYouNeed': 'आवश्यक कागदपत्रे',
    'postDoubt': 'शंका पोस्ट करा',
    'wrongCaptcha': 'चुकीचा कॅप्चा',
    'postReported': 'रिपोर्ट केले. पोस्ट लपवली आहे.',
    'markSolved': 'सोडवले म्हणून चिन्हांकित करा',
    'report': 'रिपोर्ट करा',
};

const ta = {
    'Manage IDs': 'IDகளை நிர்வகிக்கவும்',
    'Verify ID': 'IDஐச் சரிபார்க்கவும்',
    'Download Card': 'அட்டையைப் பதிவிறக்கவும்',
    'Create New ID Card': 'புதிய அடையாள அட்டையை உருவாக்கவும்',
    'Fill in the details to generate a new digital ID card.': 'புதிய டிஜிட்டல் அடையாள அட்டையை உருவாக்க விவரங்களைப் பூர்த்தி செய்யவும்।',
    'Full Name': 'முழு பெயர்',
    'Date of Birth': 'பிறந்த தேதி',
    'Gender': 'பாலினம்',
    'Male': 'ஆண்',
    'Female': 'பெண்',
    'Other': 'மற்றவை',
    'Address': 'முகவரி',
    'Photo': 'புகைப்படம்',
    'Click to upload': 'பதிவேற்ற கிளிக் செய்யவும்',
    'Generate ID Card': 'அடையாள அட்டையை உருவாக்கவும்',
    'My ID Cards': 'எனது அடையாள அட்டைகள்',
    'A list of all the digital ID cards you have created.': 'நீங்கள் உருவாக்கிய அனைத்து டிஜிட்டல் அடையாள அட்டைகளின் பட்டியல்।',
    "You haven't created any ID cards yet.": "நீங்கள் இன்னும் எந்த அடையாள அட்டைகளையும் உருவாக்கவில்லை।",
    'Search by ID Number...': 'ஐடி எண் மூலம் தேடவும்...',
    'No cards found for your search.': 'உங்கள் தேடலுக்கு எந்த அட்டைகளும் கிடைக்கவில்லை।',
    'siteTitle': 'யோஜனா மித்ரா • மகாராஷ்டிரா',
    'siteTag': 'திட்டங்கள் மற்றும் சமூக உதவி',
    'heroTitle': 'நிமிடங்களில் சரியான திட்டத்தைக் கண்டறியவும்',
    'heroSub': 'நீங்கள் விண்ணப்பிக்கக்கூடிய திட்டங்களைக் காண சில கேள்விகளுக்கு பதிலளிக்கவும்.',
    'heroNote': 'உள்நுழைவு தேவையில்லை. அநாமதேய இடுகையிடல் அனுமதிக்கப்படுகிறது.',
    'finderTitle': 'ஸ்மார்ட் திட்டம் கண்டுபிடிப்பான்',
    'labelDistrict': 'மாவட்டம்',
    'labelCrop': 'பயிர்',
    'labelLand': 'நில அளவு',
    'labelIrr': 'நீர்ப்பாசனம்',
    'labelAge': 'வயது',
    'labelKyc': 'கேஒய்சி மற்றும் வங்கி இணைப்பு',
    'btnShowEligible': 'தகுதியான திட்டங்களைக் காட்டு',
    'btnClear': 'தெளிவுபடுத்து',
    'btnFindSchemes': 'திட்டங்களைக் கண்டறியவும்',
    'btnAsk': 'சந்தேகம் கேளுங்கள்',
    'qaTitle': "விவசாயிகளின் கேள்வி பதில்",
    'labelQDistrict': 'மாவட்டம்',
    'labelQCat': 'வகை',
    'labelQTitle': 'தலைப்பு',
    'labelQBody': 'உங்கள் சிக்கலை விவரிக்கவும்',
    'lblUnanswered': 'பதிலளிக்கப்படாதது',
    'print': 'அச்சிடுக',
    'apply': 'ஆன்லைனில் விண்ணப்பிக்கவும்',
    'footerTxt': 'சிறந்த முயற்சித் தகவல். அதிகாரப்பூர்வ ஆதாரங்களுடன் சரிபார்க்கவும்.',
    'details': 'விவரங்கள்',
    'noSchemes': 'பொருத்தமான திட்டங்கள் எதுவும் கிடைக்கவில்லை. வடிப்பான்களை மாற்ற முயற்சிக்கவும்.',
    'noPosts': 'இன்னும் பதிவுகள் இல்லை.',
    'captcha': 'கேப்ட்சா:',
    'copy': 'நகலெடு',
    'copied': 'நகலெடுக்கப்பட்டது!',
    'downloadPdf': 'PDF பதிவிறக்கவும்',
    'close': 'மூடு',
    'whatYouGet': 'உங்களுக்கு என்ன கிடைக்கும்',
    'eligibility': 'தகுதி',
    'howToApply': 'விண்ணப்பிப்பது எப்படி',
    'docsYouNeed': 'தேவையான ஆவணங்கள்',
    'postDoubt': 'சந்தேகத்தை இடுகையிடவும்',
    'wrongCaptcha': 'தவறான கேப்ட்சா',
    'postReported': 'புகாரளிக்கப்பட்டது. இடுகை மறைக்கப்பட்டது.',
    'markSolved': 'தீர்க்கப்பட்டதாகக் குறிக்கவும்',
    'report': 'புகாரளி',
};

const gu = {
    'Manage IDs': 'ID મેનેજ કરો',
    'Verify ID': 'ID ચકાસો',
    'Download Card': 'કાર્ડ ડાઉનલોડ કરો',
    'Create New ID Card': 'નવું ID કાર્ડ બનાવો',
    'Fill in the details to generate a new digital ID card.': 'નવું ડિજિટલ ID કાર્ડ જનરેટ કરવા માટે વિગતો ભરો।',
    'Full Name': 'પૂરું નામ',
    'Date of Birth': 'જન્મ તારીખ',
    'Gender': 'જાતિ',
    'Male': 'પુરુષ',
    'Female': 'સ્ત્રી',
    'Other': 'અન્ય',
    'Address': 'સરનામું',
    'Photo': 'ફોટો',
    'Click to upload': 'અપલોડ કરવા માટે ક્લિક કરો',
    'Generate ID Card': 'ID કાર્ડ જનરેટ કરો',
    'My ID Cards': 'મારા ID કાર્ડ્સ',
    'A list of all the digital ID cards you have created.': 'તમે બનાવેલા તમામ ડિજિટલ ID કાર્ડ્સની સૂચિ।',
    "You haven't created any ID cards yet.": "તમે હજી સુધી કોઈ ID કાર્ડ બનાવ્યું નથી।",
    'Search by ID Number...': 'આઈડી નંબર દ્વારા શોધો...',
    'No cards found for your search.': 'તમારી શોધ માટે કોઈ કાર્ડ મળ્યાં નથી।',
    'siteTitle': 'યોજના મિત્ર • મહારાષ્ટ્ર',
    'siteTag': 'યોજનાઓ અને સમુદાય સહાય',
    'heroTitle': 'મિનિટોમાં યોગ્ય યોજના શોધો',
    'heroSub': 'તમે અરજી કરી શકો તે યોજનાઓ જોવા માટે થોડા પ્રશ્નોના જવાબ આપો.',
    'heroNote': 'કોઈ લૉગિન જરૂરી નથી. અનામી પોસ્ટિંગની મંજૂરી છે.',
    'finderTitle': 'સ્માર્ટ યોજના શોધક',
    'labelDistrict': 'જિલ્લો',
    'labelCrop': 'પાક',
    'labelLand': 'જમીનનું કદ',
    'labelIrr': 'સિંચાઈ',
    'labelAge': 'ઉંમર',
    'labelKyc': 'કેવાયસી અને બેંક લિંક',
    'btnShowEligible': 'પાત્ર યોજનાઓ બતાવો',
    'btnClear': 'સાફ કરો',
    'btnFindSchemes': 'યોજનાઓ શોધો',
    'btnAsk': 'શંકા પૂછો',
    'qaTitle': "ખેડૂતોના પ્રશ્નો અને જવાબો",
    'labelQDistrict': 'જિલ્લો',
    'labelQCat': 'શ્રેણી',
    'labelQTitle': 'શીર્ષક',
    'labelQBody': 'તમારી સમસ્યાનું વર્ણન કરો',
    'lblUnanswered': 'જવાબ ન અપાયેલ',
    'print': 'છાપો',
    'apply': 'ઑનલાઇન અરજી કરો',
    'footerTxt': 'શ્રેષ્ઠ-પ્રયાસ માહિતી. સત્તાવાર સ્ત્રોતો સાથે ચકાસો.',
    'details': 'વિગતો',
    'noSchemes': 'કોઈ મેળ ખાતી યોજનાઓ મળી નથી. ફિલ્ટર્સ બદલવાનો પ્રયાસ કરો.',
    'noPosts': 'હજી સુધી કોઈ પોસ્ટ નથી.',
    'captcha': 'કેપ્ચા:',
    'copy': 'કૉપિ કરો',
    'copied': 'કૉપિ કરેલ!',
    'downloadPdf': 'પીડીએફ ડાઉનલોડ કરો',
    'close': 'બંધ કરો',
    'whatYouGet': 'તમને શું મળે છે',
    'eligibility': 'પાત્રતા',
    'howToApply': 'કેવી રીતે અરજી કરવી',
    'docsYouNeed': 'જરૂરી દસ્તાવેજો',
    'postDoubt': 'શંકા પોસ્ટ કરો',
    'wrongCaptcha': 'ખોટો કેપ્ચા',
    'postReported': ' જાણ કરાઈ. પોસ્ટ છુપાવેલ છે.',
    'markSolved': 'ઉકેલાયેલ તરીકે ચિહ્નિત કરો',
    'report': 'જાણ કરો',
};

const kn = {
    'Manage IDs': 'ID ಗಳನ್ನು ನಿರ್ವಹಿಸಿ',
    'Verify ID': 'ID ಯನ್ನು ಪರಿಶೀಲಿಸಿ',
    'Download Card': 'ಕಾರ್ಡ್ ಡೌನ್‌ಲೋಡ್ ಮಾಡಿ',
    'Create New ID Card': 'ಹೊಸ ID ಕಾರ್ಡ್ ರಚಿಸಿ',
    'Fill in the details to generate a new digital ID card.': 'ಹೊಸ ಡಿಜಿಟಲ್ ID ಕಾರ್ಡ್ ರಚಿಸಲು ವಿವರಗಳನ್ನು ಭರ್ತಿ ಮಾಡಿ।',
    'Full Name': 'ಪೂರ್ಣ ಹೆಸರು',
    'Date of Birth': 'ಹುಟ್ಟಿದ ದಿನಾಂಕ',
    'Gender': 'ಲಿಂಗ',
    'Male': 'ಪುರುಷ',
    'Female': 'ಮಹಿಳೆ',
    'Other': 'ಇತರೆ',
    'Address': 'ವಿಳಾಸ',
    'Photo': 'ಫೋಟೋ',
    'Click to upload': 'ಅಪ್‌ಲೋಡ್ ಮಾಡಲು ಕ್ಲಿಕ್ ಮಾಡಿ',
    'Generate ID Card': 'ID ಕಾರ್ಡ್ ರಚಿಸಿ',
    'My ID Cards': 'ನನ್ನ ID ಕಾರ್ಡ್‌ಗಳು',
    'A list of all the digital ID cards you have created.': 'ನೀವು ರಚಿಸಿದ ಎಲ್ಲಾ ಡಿಜಿಟಲ್ ID ಕಾರ್ಡ್‌ಗಳ ಪಟ್ಟಿ।',
    "You haven't created any ID cards yet.": "ನೀವು ಇನ್ನೂ ಯಾವುದೇ ID ಕಾರ್ಡ್‌ಗಳನ್ನು ರಚಿಸಿಲ್ಲ।",
    'Search by ID Number...': 'ಐಡಿ ಸಂಖ್ಯೆಯ ಮೂಲಕ ಹುಡುಕಿ...',
    'No cards found for your search.': 'ನಿಮ್ಮ ಹುಡುಕಾಟಕ್ಕೆ ಯಾವುದೇ ಕಾರ್ಡ್‌ಗಳು ಕಂಡುಬಂದಿಲ್ಲ।',
    'siteTitle': 'ಯೋಜನಾ ಮಿತ್ರ • ಮಹಾರಾಷ್ಟ್ರ',
    'siteTag': 'ಯೋಜನೆಗಳು ಮತ್ತು ಸಮುದಾಯ ಸಹಾಯ',
    'heroTitle': 'ನಿಮಿಷಗಳಲ್ಲಿ ಸರಿಯಾದ ಯೋಜನೆಯನ್ನು ಹುಡುಕಿ',
    'heroSub': 'ನೀವು ಅರ್ಜಿ ಸಲ್ಲಿಸಬಹುದಾದ ಯೋಜನೆಗಳನ್ನು ನೋಡಲು ಕೆಲವು ಪ್ರಶ್ನೆಗಳಿಗೆ ಉತ್ತರಿಸಿ.',
    'heroNote': 'ಯಾವುದೇ ಲಾಗಿನ್ ಅಗತ್ಯವಿಲ್ಲ. ಅನಾಮಧೇಯ ಪೋಸ್ಟಿಂಗ್‌ಗೆ ಅನುಮತಿಸಲಾಗಿದೆ.',
    'finderTitle': 'ಸ್ಮಾರ್ಟ್ ಸ್ಕೀಮ್ ಫೈಂಡರ್',
    'labelDistrict': 'ಜಿಲ್ಲೆ',
    'labelCrop': 'ಬೆಳೆ',
    'labelLand': 'ಭೂಮಿಯ ಗಾತ್ರ',
    'labelIrr': 'ನೀರಾವರಿ',
    'labelAge': 'ವಯಸ್ಸು',
    'labelKyc': 'ಕೆವೈಸಿ ಮತ್ತು ಬ್ಯಾಂಕ್ ಲಿಂಕ್',
    'btnShowEligible': 'ಯೋಗ್ಯ ಯೋಜನೆಗಳನ್ನು ತೋರಿಸಿ',
    'btnClear': 'ತೆರವುಗೊಳಿಸಿ',
    'btnFindSchemes': 'ಯೋಜನೆಗಳನ್ನು ಹುಡುಕಿ',
    'btnAsk': 'ಸಂದೇಹವನ್ನು ಕೇಳಿ',
    'qaTitle': "ರೈತರ ಪ್ರಶ್ನೋತ್ತರ",
    'labelQDistrict': 'ಜಿಲ್ಲೆ',
    'labelQCat': 'ವರ್ಗ',
    'labelQTitle': 'ಶೀರ್ಷಿಕೆ',
    'labelQBody': 'ನಿಮ್ಮ ಸಮಸ್ಯೆಯನ್ನು ವಿವರಿಸಿ',
    'lblUnanswered': 'ಉತ್ತರಿಸದ',
    'print': 'ಮುದ್ರಿಸು',
    'apply': 'ಆನ್‌ಲೈನ್‌ನಲ್ಲಿ ಅನ್ವಯಿಸಿ',
    'footerTxt': 'ಉತ್ತಮ-ಪ್ರಯತ್ನದ ಮಾಹಿತಿ. ಅಧಿಕೃತ ಮೂಲಗಳೊಂದಿಗೆ ಪರಿಶೀಲಿಸಿ.',
    'details': 'ವಿವರಗಳು',
    'noSchemes': 'ಯಾವುದೇ ಹೊಂದಾಣಿಕೆಯ ಯೋಜನೆಗಳು ಕಂಡುಬಂದಿಲ್ಲ. ಫಿಲ್ಟರ್‌ಗಳನ್ನು ಬದಲಾಯಿಸಲು ಪ್ರಯತ್ನಿಸಿ.',
    'noPosts': 'ಇನ್ನೂ ಯಾವುದೇ ಪೋಸ್ಟ್‌ಗಳಿಲ್ಲ.',
    'captcha': 'ಕ್ಯಾಪ್ಚಾ:',
    'copy': 'ನಕಲಿಸಿ',
    'copied': 'ನಕಲಿಸಲಾಗಿದೆ!',
    'downloadPdf': 'ಪಿಡಿಎಫ್ ಡೌನ್‌ಲೋಡ್ ಮಾಡಿ',
    'close': 'ಮುಚ್ಚಿ',
    'whatYouGet': 'ನೀವು ಏನು ಪಡೆಯುತ್ತೀರಿ',
    'eligibility': 'ಅರ್ಹತೆ',
    'howToApply': 'ಅನ್ವಯಿಸುವುದು ಹೇಗೆ',
    'docsYouNeed': 'ಅಗತ್ಯವಿರುವ ದಾಖಲೆಗಳು',
    'postDoubt': 'ಸಂದೇಹವನ್ನು ಪೋಸ್ಟ್ ಮಾಡಿ',
    'wrongCaptcha': 'ತಪ್ಪು ಕ್ಯಾಪ್ಚಾ',
    'postReported': 'ವರದಿ ಮಾಡಲಾಗಿದೆ. ಪೋಸ್ಟ್ ಮರೆಮಾಡಲಾಗಿದೆ.',
    'markSolved': 'ಪರಿಹರಿಸಲಾಗಿದೆ ಎಂದು ಗುರುತಿಸಿ',
    'report': 'ವರದಿ ಮಾಡಿ',
};

export const translations = {
  en,
  hi,
  bn,
  te,
  mr,
  ta,
  gu,
  kn,
};

export type TranslationKey = keyof typeof en;
